...
除 Reference Audit 的明确可空字段外，所有文本字段均为非空字符串，数组元素也是非空字符串。`requested`、`submitted`、`applied`、`reason` 与 `proof_media_ref` 可为 `null`，但 `dropped` 或 `blocked` 的 audit 必须有非空 `reason`；`accepted` 的 audit 必须有非空 `requested` 与 `applied`。列表中的 `facts_sources.id`、`reference_audits.audit_id`、`shot_cards.shot_id`、`cue_sheet.cue_id` 必须在各自命名空间唯一；`shot_cards.reference_audit_refs` 必须引用同一快照中的 `audit_id`，`cue_sheet.shot_id` 必须引用同一快照中的 `shot_cards.shot_id`。`production_profile` 只能为 `html_launch` 或 `t2v_brand_film`。`reference_audits.subject_type` 只能为 `product`、`person`、`scene`、`style`、`first_frame`、`last_frame` 或 `proof_media`；其 `requirement` 为 `optional` 或 `required`，`outcome` 为 `not_requested`、`accepted`、`dropped` 或 `blocked`。`cue_sheet.kind` 为 `vo`、`sfx`、`caption`、`cta`、`transition` 或 `hold`。`decision` 只能为 `accepted`、`revise` 或 `rejected`。`score` 与 `confidence` 均为 0 到 1 的数值。时间区间必须是非负且 `end_seconds >= start_seconds`。
