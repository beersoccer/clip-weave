...
6. `reference_audit` 的 `requested` 保留用户原始 reference；`applied` 为实际提交 URI。额外写入 `proof_media` 快照（sha256、uri、source、可选 notes），使恢复指纹随实际提交 URI 改变。
...
1. 本地 required reference 经 fake HTTP store 上传后以支持的 URI 提交，并在 manifest/proof-media ledger 中留下 SHA-256 记录。
