...
**本 spec 覆盖范围：**
- Phase 1：video_analyzer（FFmpeg + LLM 多模态分析）→ shots.json（约 1 周）
...
选型依据：适配层将底层工具与业务逻辑解耦，使各阶段可独立测试和替换，同时保持结构轻量。
...
│   ├── adapters/
│   │   ├── video_analyzer.py    # FFmpeg 帧提取 + LLM 多模态分析
│   │   └── hyperframes.py       # HyperFrames CLI 封装
│   ├── core/
...
```
VIDEO_ANALYSIS_BASE_URL=   # OpenAI 兼容接口，支持公司网关
...
sample.mp4
    │
    ▼  adapters/video_analyzer.py
    │  FFmpeg 关键帧提取 → LLM 多模态分析（OpenAI 兼容接口）
...
| 阶段 | 失败场景 | 处理方式 |
|---|---|---|
| video_analyzer 调用 | FFmpeg 或 LLM API 错误 | 抛出 `VideoAnalysisError`，附带 stderr 和 LLM 原始响应 |
...
| Adapters | `video_analyzer.py` / `hyperframes.py` | Mock LLM 和 FFmpeg 调用，验证入参和输出解析 |
