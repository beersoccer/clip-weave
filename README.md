...
```dotenv
GEMINI_API_KEY=your_gemini_key          # 必填：视频帧分析
ANTHROPIC_API_KEY=your_anthropic_key    # 必填（默认模型 claude）
OPENAI_API_KEY=                         # 可选：HTML_GEN_MODEL=gpt4o 时才需要
PEXELS_API_KEY=                         # 可选：缺失时跳过素材搜索，不影响主流程
HTML_GEN_MODEL=claude                   # 可选：claude（默认）或 gpt4o
...
先单独跑分析，确认 Gemini 和 FFmpeg 工作正常，再做完整渲染：
...
uv run python -m clip_weave run \
  ...
  [--html-model claude|gpt4o]   # 覆盖 HTML_GEN_MODEL 环境变量
```
...
**Q: `VideoAnalysisError: Gemini API call failed`**  
A: 检查 `.env` 中 `GEMINI_API_KEY` 是否正确，以及网络是否可访问 Gemini API。
